    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if (sender instanceof Player player) {
            ...
            switch (subCommand) {
                ...
                case "count" ->
                        sender.sendMessage("Il y a " + KrimsonAPI.customBlocks.getGlobalContainer().getAllBlocks().size() + " blocs custom chargés.\nTické:\n" +
                                " - Async: " + KrimsonAPI.customBlocks.getLastTickedCount().getOrDefault(true, 0) + "\n" +
                                " - Sync: " + KrimsonAPI.customBlocks.getLastTickedCount().getOrDefault(false, 0) + "\n" +
