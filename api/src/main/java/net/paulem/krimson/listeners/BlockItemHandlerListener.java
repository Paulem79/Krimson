...
public class BlockItemHandlerListener implements Listener {
    ...
    public void onBlockPlace(PlayerInteractEvent event) {
        ...
        if (key == null) {
            KrimsonPlugin.getInstance().getLogger().warning("Invalid item identifier: " + identifier + " for item: " + item.getType() + " used by player: " + player.getName());
            return;
        }
        ...
        if(!(blockItem instanceof CustomBlockItem customBlockItem)) {
            KrimsonPlugin.getInstance().getLogger().warning("Item " + blockItem.getKey() + " is not a CustomBlockItem, cannot place it as a block.");
            return;
        }
    ...
    public void onTryCraft(PrepareItemCraftEvent event) {
        ...
        if (Arrays.stream(matrix).anyMatch(item -> {
            ...
            if (key == null) {
                KrimsonPlugin.getInstance().getLogger().warning("Invalid item identifier: " + identifier + " for item: " + item.getType() + " used by player: " + event.getView().getPlayer().getName());
                return false;
            }
